...
class EnergyPlusEnv(Env):
    ...
    # Overrides the seed function from the gym environment
    ...
    def stop_instance(self):
        ...
        if self.episode_idx >= 0:
            def count_severe_errors(file):
                ...
                # Sample: '   ************* EnergyPlus Completed Successfully-- 6214 Warning; 2 Severe Errors;
                # Elapsed Time=00hr 00min  7.19sec'
                fd = open(file)
                lines = fd.readlines()
                fd.close()
                for line in lines:
    ...
    def step(self, action):
        ...
        # Receive observation from the environment Note that in our co-simulation environment, the state value of the
        # last time step can not be retrived from EnergyPlus process because EMS framework of EnergyPlus does not
        # allow setting EMS calling point ater the last timestep is completed. To remedy this, we assume
        # set_raw_state() method of each model handle the case raw_state is None.
...
def parser():
    usage = 'Usage: python {} [--verbose] [--energyplus <file>] [--model <file>] [--weather <file>] [--simulate] [' \
            '--plot] [--help]'.format(__file__)
    ...
    if args.simulate:
        for ep in range(1):
            ...
            for i in range(1000000):
                ...
                if args.verbose:
                    print('========= count={} PUE={} PUEave={} PUEmin={} PUEmax={}'.format(PUE_count, PUE, PUE_ave,
                                                                                           PUE_min, PUE_max))
                if done:
                    break
            PUE_ave = PUE_sum / PUE_count
            print('============================= Episode done. count={} PUEave={} PUEmin={} PUEmax={}'.format(PUE_count,
                                                                                                              PUE_ave,
                                                                                                              PUE_min,
                                                                                                              PUE_max))
            # env.close()
