...
class EnergyPlusModel2ZoneDataCenterHVAC_wEconomizer_Temp_Fan(EnergyPlusModel):
    ...
    def __init__(self,
        ...
        self.electric_powers = [
            ...
            # 'WESTDATACENTER_EQUIP:ITE CPU Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            # 'WESTDATACENTER_EQUIP:ITE Fan Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            ...
            # 'WEST ZONE:Zone ITE CPU Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            # 'WEST ZONE:Zone ITE Fan Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            # 'WEST DATA CENTER IEC:Evaporative Cooler Electric Power [W](TimeStep)', #
            #       low (works only on very cold day)
            ...
            # 'EASTDATACENTER_EQUIP:ITE CPU Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            # 'EASTDATACENTER_EQUIP:ITE Fan Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            ...
            # 'EAST ZONE:Zone ITE CPU Electric Power at Design Inlet Conditions [W](Hourly)',
            #       low (only depends on time in day)
            # 'EAST ZONE:Zone ITE Fan Electric Power at Design Inlet Conditions [W](Hourly)', #
            #       low (only depends on time in day)
            # 'EAST DATA CENTER IEC:Evaporative Cooler Electric Power [W](TimeStep)', #
            #       low (works only on very cold day)
    ...
    def compute_reward_common(self,
        ...
        # Temp. gaussian
        rew_temp_gaussian1 = np.exp(-(Tz1 - temperature_center) * (
                    Tz1 - temperature_center) * temperature_gaussian_sharpness) * temperature_gaussian_weight
        rew_temp_gaussian2 = np.exp(-(Tz2 - temperature_center) * (
                    Tz2 - temperature_center) * temperature_gaussian_sharpness) * temperature_gaussian_weight
        ...
        if os.path.exists("/tmp/verbose"):
            print(
                'compute_reward: rew={:7.3f} (temp_gaussian1={:7.3f}, temp_gaussian2={:7.3f}, temp_trapezoid1={'
                ':7.3f}, temp_trapezoid2={:7.3f}, fluct={:7.3f}, PUE={:7.3f}, Power={:7.3f})'.format(
                    rew, rew_temp_gaussian1, rew_temp_gaussian2, rew_temp_trapezoid1, rew_temp_trapezoid2, rew_fluct,
                    rew_PUE, rew_Whole_Building_Power))
        if os.path.exists("/tmp/verbose2"):
            print(
                'compute_reward: Tenv={:7.3f}, Tz1={:7.3f}, Tz2={:7.3f}, PUE={:7.3f}, Whole_Powerd2={:8.1f}, '
                'ITE_Power={:8.1f}, HVAC_Power={:8.1f}, Act1={:7.3f}, Act2={:7.3f}, Act3={:7.3f}, Act4={:7.3f}'.format(
                    Tenv, Tz1, Tz2, PUE, Whole_Building_Power, IT_Equip_Power, Whole_HVAC_Power, self.action[0],
                    self.action[1], self.action[2], self.action[3]))

        return rew, (
        rew_temp_gaussian1, rew_temp_trapezoid1, rew_temp_gaussian2, rew_temp_trapezoid2, rew_Whole_Building_Power)
    ...
    def read_episode(self, ep):
        ...
        self.westzone_supply_fan_outlet_temp = df['WEST ZONE SUPPLY FAN OUTLET NODE:System Node Temperature [C](' \
                                                  'TimeStep)']
        self.westzone_dec_outlet_temp = df['WEST ZONE DEC OUTLET NODE:System Node Temperature [C](TimeStep)']
        self.westzone_dec_outlet_setpoint_temp = df[
        ...
        self.westzone_air_loop_outlet_setpoint_temp = df[
        ...
        self.eastzone_supply_fan_outlet_temp = df[
        ...
        self.eastzone_dec_outlet_setpoint_temp = df[
        ...
        for Tenv, Tz1, Tz2, PUE, Whole_Building_Power, IT_Equip_Power, Whole_HVAC_Power in zip(
            ...
            rew, elem = self._compute_reward(
                [Tenv, Tz1, Tz2, PUE, Whole_Building_Power, IT_Equip_Power, Whole_HVAC_Power])
    ...
    def plot_episode(self, ep):
        ...
        self.show_statistics('Power consumption',
            ...
            for i in range(self.num_axes):
                if i == 0:
                    ax = self.fig.add_axes(
                        [0.05, 1.00 - 0.70 / self.num_axes * (i + 1), 0.90, 0.7 / self.num_axes * 0.85])
                else:
                    ax = self.fig.add_axes(
                        [0.05, 1.00 - 0.70 / self.num_axes * (i + 1), 0.90, 0.7 / self.num_axes * 0.85],
                        sharex=self.axepisode[0])
                ax.set_xmargin(0)
                self.axepisode.append(ax)
                ax.set_xticks(self.x_pos)
                ax.set_xticklabels(self.x_labels[0::600])
        ...
        if True:
            ...
            idx += 1
            ax.lines = []
            ax.plot(self.df['WEST ZONE SUPPLY FAN:Fan Air Mass Flow Rate [kg/s](Hourly)'], 'C0',
                    label='WEST ZONE Air Mass Flow Rate')
            ax.plot(self.df['EAST ZONE SUPPLY FAN:Fan Air Mass Flow Rate [kg/s](Hourly)'], 'C1',
                    label='EAST ZONE Air Mass Flow Rate')
            ax.legend()
        ...
        if True:
            ...
            if show_west:
                ...
                ax.plot(self.westzone_supply_fan_outlet_temp, 'C2',
                        label='WEST ZONE SUPPLY FAN OUTLET NODE:System Node Temperature')
                ...
                ax.plot(self.eastzone_supply_fan_outlet_temp, 'C2',
                        label='EAST ZONE SUPPLY FAN OUTLET NODE:System Node Temperature')
        ...
        if True:
            ...
            if show_west:
                ...
                ax.plot(self.westzone_dec_outlet_setpoint_temp, label='Westzone DEC outlet setpoint temperature',
                ...
                ax.plot(self.eastzone_dec_outlet_setpoint_temp, label='Eastzone DEC outlet setpoint temperature',
                        linewidth=0.5, color='gray')
            ax.plot(self.cooling_setpoint, label='Cooling setpoint', linestyle='--', linewidth=0.5, color='blue')
            ax.plot(self.heating_setpoint, label='Heating setpoint', linestyle='--', linewidth=0.5, color='red')
            ax.legend()
        ...
        if True:
            ...
            ax.lines = []
            ax.plot(self.total_electric_demand_power, 'C0', label='Whole Building:Facility Total Electric Demand Power')
            ax.plot(self.total_building_electric_demand_power, 'C1',
                    label='Whole Building:Facility Total Building Electric Demand Power')
            ax.plot(self.total_hvac_electric_demand_power, 'C2',
                    label='Whole Building:Facility Total HVAC Electric Demand Power')
            ax.legend()
            ax.set_ylabel('Power (W)')
        ...
        self.text_power_consumption.append(self.fig.text(0.02, 0.25, 'Whole Power:    {:6,.1f} kW'.format(
            np.average(self.df['Whole Building:Facility Total Electric Demand Power [W](Hourly)']) / 1000)))
        self.text_power_consumption.append(self.fig.text(0.02, 0.235, 'Building Power: {:6,.1f} kW'.format(
            np.average(self.df['Whole Building:Facility Total Building Electric Demand Power [W](Hourly)']) / 1000)))
        self.text_power_consumption.append(self.fig.text(0.02, 0.22, 'HVAC Power:     {:6,.1f} kW'.format(
            np.average(self.df['Whole Building:Facility Total HVAC Electric Demand Power [W](Hourly)']) / 1000)))
    ...
    def dump_episodes(self, log_dir='', csv_file='', **kwargs):
        ...
        with open('dump_episodes.dat', mode='w') as f:
            ...
            for ep in range(self.num_episodes):
                ...
                Power, _, _, _ = self.get_statistics(
                    self.df['Whole Building:Facility Total Electric Demand Power [W](Hourly)'])

                f.write(
                    '"{}" {:5.2f} {:5.2f} {:5.2f} {:4.2f} {:5.2f} {:5.2f} {:5.2f} {:4.2f} {:5.2f} {:9.2f} {:8.3%} {:8.3%} {:3d}\n'.format(
                        self.weather_key, Ave1, Min1, Max1, STD1, Ave2, Min2, Max2, STD2, Rew, Power, In22_25_1,
                        In22_25_2, ep))
